...
class Player:
    ...
    def render(self, window, segment):
        ...
        p = pygame.image.load(os.path.join(path+"lib", sprite["path"]))
...
#        font        = pygame.font.Font(s.FONTS["retro_computer"], 16)
        font        = pygame.font.Font(path+"lib/retro_computer.ttf", 16)
        ...
        if self.points > self.next_milestone and self.status == self.ALIVE:
            milestone_sfx = pygame.mixer.Sound(os.path.join(path+"lib", "excellent.ogg"))
            milestone_sfx.play()
        ...
        if self.high_score > 0 and self.points > self.high_score:
            high_score_sfx = pygame.mixer.Sound(os.path.join(path+"lib", "excellent.ogg"))
            high_score_sfx.play()
    ...
    def render_blood(self, window):
        ...
        path = rospack.get_path('uchile_fun')
        path+="/src/uchile_fun/SwervinMervin/" 
        b = pygame.image.load(os.path.join(path+"lib", "blood.png"))
    ...
    def travel(self, track_length, window):
        ...
        if self.status == self.ALIVE:
            ...
            if self.time_left <= 0:#0!!!!!!!!!!!!!!!!!!!!!!!!!#####################################
                go_sfx = pygame.mixer.Sound(os.path.join(path+"lib", "loser.ogg"))
                go_sfx.play()
        ...
        if pos >= track_length:
            ...
            if self.status == self.ALIVE and self.lap == self.total_laps:
                ...
                lap_sfx = pygame.mixer.Sound(os.path.join(path+"lib", "570.wav"))
                lap_sfx.play()
            ...
            if self.status != self.GAME_OVER:
                ...
                if self.__fastest_lap():
                    if self.lap > 2:
                        ...
                        fast_lap_sfx = pygame.mixer.Sound(os.path.join(path+"lib", "jim.ogg"))
                        fast_lap_sfx.play()
...
#        rospy.init_node('listener', anonymous=True)
    ...
    def __hit_hooker(self):
        path = rospack.get_path('uchile_fun')
        path+="/src/uchile_fun/SwervinMervin/" 
        crash_sfx        = pygame.mixer.Sound(os.path.join(path+"lib", "scream.ogg"))
        splat_sfx        = pygame.mixer.Sound(os.path.join(path+"lib", "blood.ogg"))
    ...
    def __hit_bonus(self):
        ...
        bonus_sfx = pygame.mixer.Sound(os.path.join(path+"lib", "oh_yeah.ogg"))
        bonus_sfx.play()
    ...
    def __hit_speed_boost(self):
        ...
        path+="/src/uchile_fun/SwervinMervin/"
        if self.speed_boost == 1:
            boost_sfx = pygame.mixer.Sound(os.path.join(path+"lib", "speed_boost.ogg"))
            boost_sfx.play()
    ...
    def __hit_world_object(self):
        ...
        crash_sfx        = pygame.mixer.Sound(os.path.join(path+"lib", "you_fool.ogg"))
        self.crashed     = True
        self.speed       = 0
        self.speed_boost = 1
    ...
    def __hit_competitor(self):
        path = rospack.get_path('uchile_fun')
        path+="/src/uchile_fun/SwervinMervin/"
        crash_sfx = pygame.mixer.Sound(os.path.join(path+"lib", "car_crash.ogg"))
        crash_sfx.play()
    ...
    def __run_screech(self):
        ...
        if not self.screech_sfx:
            self.screech_sfx = pygame.mixer.Sound(os.path.join(path+"lib", "screech_short.ogg"))
    ...
    def __game_over_overlay(self, window):
        path = rospack.get_path('uchile_fun')
        path+="/src/uchile_fun/SwervinMervin/"
        go_font = pygame.font.Font(path+"lib/retro_computer.ttf", 44)  
    ...
    def __level_over_overlay(self, window):
        ...
        lo_font      = pygame.font.Font(path+"lib/fipps.ttf", 38)
        s_font       = pygame.font.Font(path+"lib/retro_computer.ttf", 30)
...
#rospy.loginfo(rospy.get_caller_id() + "I heard %s", Joy)
    ...
    global RIGHT_LEFT, UP_DOWN, EXIT
