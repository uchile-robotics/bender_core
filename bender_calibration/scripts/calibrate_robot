...
# Copyright (C) 2015 bender Robotics Inc
...
if __name__ == "__main__":
    ...
    if args.reset:
        # Can we get to bender_bringup?
        try:
            rp = rospkg.RosPack()
            bringup = rp.get_path("bender_bringup")
        except:
            print("\nCould not find bender_bringup package, is your ROS path correct?\n")
            exit(-1)
        # Reset the robot.launch to factory defaults
        copy_file(os.path.join(bringup, "launch/bender.launch"), etc_launch)
        restart = True
    ...
    if args.arm:
        try:
            ...
            exit(-1)
        if subprocess.call(["roslaunch", "bender_calibration", "capture.launch"]) != 0:
