...
class TTSSkill(RobotSkill):
	...
	def setup(self):
		...
		self._stop_tts_client = rospy.ServiceProxy(self._stop_tts_topic, String)
		return True
	...
	def stop(self):
		rospy.loginfo("{skill: %s}: stop()." % self._type)
		"""
		Send speech recognition command to start the recognition
