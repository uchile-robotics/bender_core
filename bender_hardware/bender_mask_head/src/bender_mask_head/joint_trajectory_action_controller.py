...
# Based on Antons Rebguns joint trajectory controller
...
class JointTrajectoryActionController(object):
	def __init__(self, controller_namespace, controllers):
		...
		self.joint_state_msg.position = [0.0]*self.num_joints
		self.joint_state_msg.velocity = [0.0]*self.num_joints
		self.joint_state_msg.effort = [0.0]*self.num_joints
	...
	def initialize(self):
		...
		self.joint_states_topic = rospy.get_param(self.controller_namespace + '/joint_trajectory_action_node/joint_states_topic',
			'/joint_states')
	...
	def update_state(self):
		...
		while self.running and not rospy.is_shutdown():
			...
			for i, joint in enumerate(self.joint_names):
				...
				# Joint state message
				self.joint_state_msg.position[i] = state.current_pos
				self.joint_state_msg.velocity[i] = state.velocity
				self.joint_state_msg.effort[i] = state.load
